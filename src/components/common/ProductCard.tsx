...
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  const handleSellAction = async (e: React.MouseEvent) => {
    ...
    if (existingResellerProduct) {
      // If already added, offer to open landing page or detail view
      navigate('product-detail', { productId: product.id });
      return;
    }
    ...
    >
      ...
      <div className="relative w-full aspect-square bg-[#F7F6F3] p-3 sm:p-4 flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5 items-start">
          {isAuthenticated ? (
            <div className="px-2.5 py-1 rounded-lg bg-emerald-600 text-white text-[11px] sm:text-xs font-bold tracking-wide shadow-sm flex items-center gap-1">
              <TrendingUp className="w-3 h-3 text-emerald-200" />
              <span>৳{pricing.potentialProfit} Profit</span>
            </div>
          ) : (
            <div className="px-2 py-0.5 rounded-full bg-neutral-900/80 backdrop-blur-xs text-white text-[10px] font-semibold tracking-wide flex items-center gap-1 shadow-xs">
              <Lock className="w-2.5 h-2.5 text-amber-300" />
              <span>Login for Profit</span>
            </div>
          )}
        ...
        <div className="pt-2.5 border-t border-neutral-100 flex flex-col gap-2.5">
          ...
          {isAuthenticated ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-semibold text-neutral-500 bg-neutral-100 px-2 py-1 rounded-md">
                Profit Locked 🔒
              </span>
            </div>
          )}
          ...
          {isAuthenticated ? (
            ...
            <button
              type="button"
              onClick={handleSellAction}
              className="w-full py-2.5 px-3 rounded-xl text-xs sm:text-sm font-bold bg-[#111111] hover:bg-neutral-800 text-white flex items-center justify-center gap-1.5 transition-all shadow-xs"
            >
