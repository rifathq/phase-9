...
export function ShopView() {
  ...
  const [sortBy, setSortBy] = useState<'profit-desc' | 'popular' | 'newest' | 'price-low' | 'price-high'>('profit-desc');
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((prod) => {
      ...
      // Profit filter (only applies if authenticated)
      if (isAuthenticated && minProfitFilter > 0) {
      ...
      const pricingA = getProductResellerPricing(a, isAuthenticated);
      const pricingB = getProductResellerPricing(b, isAuthenticated);
  ...
  }, [products, selectedCategory, searchQuery, priceRange, inStockOnly, minProfitFilter, sortBy, isAuthenticated]);
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[#E6E4E0]">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              {!isAuthenticated && (
                <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-800 border border-amber-200 text-[11px] font-semibold flex items-center gap-1">
